#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_Foundable_OutsDeadBody_Oil

#include "Basic.hpp"

#include "BP_Foundable_OutsDeadBody_Oil_classes.hpp"
#include "BP_Foundable_OutsDeadBody_Oil_parameters.hpp"


SDK_NAMESPACE_START

// Function BP_Foundable_OutsDeadBody_Oil.BP_Foundable_OutsDeadBody_Oil_C.UpdateFoundableVisibilityForStorageState
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    BeginPlay_UpdateFoundableVisibilityForStorageState     (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void ABP_Foundable_OutsDeadBody_Oil_C::UpdateFoundableVisibilityForStorageState(bool BeginPlay_UpdateFoundableVisibilityForStorageState)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Foundable_OutsDeadBody_Oil_C", "UpdateFoundableVisibilityForStorageState");

	Params::BP_Foundable_OutsDeadBody_Oil_C_UpdateFoundableVisibilityForStorageState Parms{};

	Parms.BeginPlay_UpdateFoundableVisibilityForStorageState = BeginPlay_UpdateFoundableVisibilityForStorageState;

	UObject::ProcessEvent(Func, &Parms);
}


// Function BP_Foundable_OutsDeadBody_Oil.BP_Foundable_OutsDeadBody_Oil_C.OnStorageEmptied
// (Event, Public, HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    ReturnValue_OnStorageEmptied                           (Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

bool ABP_Foundable_OutsDeadBody_Oil_C::OnStorageEmptied()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("BP_Foundable_OutsDeadBody_Oil_C", "OnStorageEmptied");

	Params::BP_Foundable_OutsDeadBody_Oil_C_OnStorageEmptied Parms{};

	UObject::ProcessEvent(Func, &Parms);

	return Parms.ReturnValue;
}


SDK_NAMESPACE_END
